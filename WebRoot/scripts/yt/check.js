...
function checkForm(objFrm){
    ...
    //时间段有效
    if (f_checkStartAndEndDate(objFrm) == false) 
        return false;
...
function f_checkTextValid(obj){
    ...
    if (obj.getAttribute("isNeed") != null) {
        if (f_isNotNull(obj) == false) 
            return false;
    }
    //不能超过长度
    if (obj.getAttribute("maxlength") != null) {
        if (f_checkLength(obj) == false) 
            return false;
    }
    var checkType = "";
...
function f_checkNumType(obj, numType){
    ...
    try {
        ...
        if (numType.indexOf("(") < 0 || numType.indexOf(")") < 0) 
            return f_checkNumLenPrec(obj, defaultLen, defaultpric);
