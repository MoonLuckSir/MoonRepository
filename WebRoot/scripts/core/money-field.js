...
function initMoneyText(inpt) {
	...
	// add event listener
	inpt.onkeydown = moneyOnKeyDown;
	inpt.onkeypress = moneyOnKeyPress;
	inpt.onblur = moneyOnBlur;
}
...
function skipCursorToNext(i) {
	...
	if (inpt.readOnly || inpt.type == "hidden") {
		if ((inpt.aryIndex + skip_tabs) < money_arys.length) {
			return skipCursorToNext(inpt.aryIndex);
		}
		return null;
	}
	return inpt;
...
function skipCursorToPrev(i) {
	...
	if (inpt.readOnly || inpt.type === "hidden") {
		if (inpt.aryIndex > skip_tabs) {
			return skipCursorToPrev(inpt.aryIndex);
		}
		return null;
	}
	return inpt;
...
function moneyOnKeyDown(e) {
	...
	if ((code == 13 || code == 40) && skip_tab && skip_tabs > 0) {
		...
		var next = skipCursorToNext(inpt.aryIndex);
		if (next) {
			next.focus();
			next.select();
		}
	...
	if (code == 38 && skip_tab && skip_tabs > 0) {
		money_log("skip_tab:	" + skip_tabs);
		var next = skipCursorToPrev(inpt.aryIndex);
		if (next) {
			next.focus();
			next.select();
		}
